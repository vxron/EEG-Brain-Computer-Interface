...
ONNX_RT_C::ONNX_RT_C(){
    ...
    Ort::Env env(ORT_LOGGING_LEVEL_WARNING, "SSVEP_BCI");
    env_ = std::move(env); // no copy/assign permitted: must transfer ownership
    ...
    mem_info_ = std::move(mem_info);
    // session options - always same
...
int ONNX_RT_C::apply_softmax_to_publish_final_class(std::array<float,3> &logits){
    ...
    // get max softmax now
    float max_softmax = std::max_element(softmax_res.begin(), softmax_res.end());
    // get index of max element
    int idx_max = std::distance(softmax_res.begin(), max_softmax);
...
std::array<float, 3> ONNX_RT_C::run_inference(const sliding_window_t& window) {
    ...
    */
    ...
    Ort::Value input_tensor = Ort::Value::CreateTensor<float>(
        mem_info_,                                       // where tensor mem is allocated (set at construction time)
    ...
    std::vector<Ort::Value> input_tensor_vec = {input_tensor};
    ...
    std::vector<Ort::Value> output_tensors = loaded_model_cache_->session.Run(
        ...
        loaded_model_cache_->input_window_tensor_name, // array of input tensor names (will be size 1)
        input_tensor_vec,                              // array of input tensors (Ort::Value) (will be size 1)
        1,                                             // number of inputs
        loaded_model_cache_->output_logits_tensor_name,// array of output tensor names (will be size 1)
        1                                              // number of outputs
    )
...
bool ONNX_RT_C::load_onnx_on_new_session(std::string model_path, SettingTrainArch_E model_arch){
    ...
    */
    LoadedModel_s loaded_model_tmp{}; // temp cache
    loaded_model_tmp.model_arch = model_arch;
    loaded_model_tmp.model_path = model_path;
    ...
    Ort::Session session(env_, model_path, session_options_);
    loaded_model_tmp.session = std::move(session);
    ...
    auto in_name = session.GetInputNameAllocated(0, allocator_); // returns smart pointer for c-style string (char*) to where memory gets allocated for name
    auto out_name = session.GetOutputNameAllocated(0, allocator_);
    ...
    loaded_model_tmp.input_window_tensor_name_saved_str = std::string(in_name.get());
    loaded_model_tmp.output_logits_tensor_name_saved_str = std::string(out_name.get());
    ...
    loaded_model_tmp.input_window_tensor_name = loaded_model.input_window_tensor_name_saved_str.c_str();
    loaded_model_tmp.output_logits_tensor_name = loaded_model.output_logits_tensor_name_saved_str.c_str();

    // 4) read/verify input type + shape (for CNN: expect float32, either [1,C,T] or [1,T,C])
    auto typeInfo = session.GetInputTypeInfo(0); // type info for input 0 (only one)
    ...
    switch(model_arch){
        case "CNN": {
            loaded_model_tmp.expected_C = CNN_EXPECTED_C;
            loaded_model_tmp.expected_T = CNN_EXPECTED_T;
            loaded_model_tmp.expected_dtype = CNN_EXPECTED_INPUT_DATA_TYPE;
            break;
        }
        case "SVM": {
            break; // hadeel todo
    ...
    if(shape[1]==loaded_model_tmp.expected_C && shape[2]==loaded_model_tmp.expected_T){
        // shape [1,C,T]
        loaded_model_tmp.input_tensor_order_is_1ct = true;
        std::copy(shape.begin(), shape.end(), loaded_model_tmp.input_shape.begin());
    } 
    else if (shape[2]==loaded_model_tmp.expected_C && shape[1]==loaded_model_tmp.expected_T){
        // shape [1,T,C]
        loaded_model_tmp.input_tensor_order_is_1ct = false;
        std::copy(shape.begin(), shape.end(), loaded_model_tmp.input_shape.begin());
    }
    else {
        // shape not ok
    ...
    if(session.GetOutputTypeInfo(0))
    ...
    loaded_model_tmp.input_window_f32.resize(loaded_model_tmp.expected_C*loaded_model_tmp.expected_T);
    
    loaded_model_cache_ = &loaded_model_tmp;
