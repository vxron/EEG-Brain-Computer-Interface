...
Analyzes fold balance, batch quality, convergence, and holdout splits
"""
...
def analyze_holdout(log_text, verbose=True):
    ...
    if 'train' in data and 'hold' in data:
        ...
        hold_pct_c0 = 100 * hold['c0'] / total_c0
        hold_pct_c1 = 100 * hold['c1'] / total_c1
        hold_pct_c2 = 100 * hold['c2'] / total_c2
...
def generate_report(log_text, verbose=True):
    """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        generate_report(log_text, verbose=True)
    else:
        # Use embedded log or prompt user
        print("\nUsage options:")
        print("  python diagnose_training_v2.py <path_to_debug_log.txt>")
        print("\nOr paste your log text into the log_text variable below and run.")
        print("\nFor quick check only:")
        print("  >>> from diagnose_training_v2 import quick_check")
